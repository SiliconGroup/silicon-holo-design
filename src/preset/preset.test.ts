...
describe('presetSiliconHolo', () => {
  ...
  it('keeps the preset-only material contract aligned with the prebuilt stylesheet', () => {
    ...
    expect(css).toContain('.shd-chat-bubble{')
    expect(css).toContain('.shd-chat-bubble-assistant{')
    expect(css).toContain('.shd-chat-bubble-user{')
    expect(css).toContain('.shd-markdown-code-block{')
    expect(css).toContain('[data-shd-inline-code=true]{')
  })
})
