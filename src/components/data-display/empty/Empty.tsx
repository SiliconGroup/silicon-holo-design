...
const defaultIcon = (
  <svg className="w-12 h-12 text-content-tertiary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export function HoloEmpty({
  ...
  return (
    <div className={`flex flex-col items-center py-8 gap-3 ${className}`}>
      {icon ?? defaultIcon}
      <div className="text-content-tertiary text-sm">{resolvedDescription}</div>
      {children}
    </div>
  )
}
