...
export function HoloButton({ children, onClick, className = '', variant = 'primary', size = 'md', icon, fullWidth = false, disabled = false }: HoloButtonProps) {
  const sizeClasses = {
  ...
  const variantClasses = {
    ...
    success: 'text-status-success bg-state-success-soft border-stroke-success hover:bg-surface-interactive-hover',
    warning: 'text-status-warning bg-state-warning-soft border-stroke-warning hover:bg-surface-interactive-hover',
    danger: 'text-status-error bg-state-error-soft border-stroke-error hover:bg-surface-interactive-hover',
  }
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`rounded-sm font-medium tracking-wide flex items-center justify-center border transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2 focus-visible:ring-offset-surface-base disabled:text-content-disabled disabled:bg-transparent disabled:border-stroke-muted disabled:cursor-not-allowed ${sizeClasses[size]} ${fullWidth ? 'w-full' : ''} ${variantClasses[variant]} ${className}`}
    >
