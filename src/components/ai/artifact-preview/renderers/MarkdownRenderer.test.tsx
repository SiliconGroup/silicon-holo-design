...
describe('MarkdownRenderer', () => {
  it('loads URL Markdown and renders GFM, math, code, and Mermaid', async () => {
    ...
    const doc = container.querySelector('[data-shd-artifact-renderer="markdown"] > .shd-markdown-content')
    expect(doc).not.toBeNull()
    expect(doc?.className).toContain('shd-markdown-doc')
    expect(doc?.className).not.toContain('max-w-[78rem]')
