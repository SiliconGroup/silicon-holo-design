...
interface MessageListProps { messages: ChatMessage[]; streamingContent?: string; streamingThinking?: string; processing?: boolean }

export function MessageList({ messages, streamingContent, streamingThinking, processing }: MessageListProps) {
  const locale = useLocale()
  const containerRef = useRef<HTMLDivElement>(null)
  useEffect(() => { if (containerRef.current) containerRef.current.scrollTop = containerRef.current.scrollHeight }, [messages, streamingContent, streamingThinking])
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto px-6 py-8">
      ...
      {streamingContent && <MessageBubble message={{ id: 'streaming', role: 'assistant', content: streamingContent, timestamp: new Date().toISOString() }} isStreaming />}
